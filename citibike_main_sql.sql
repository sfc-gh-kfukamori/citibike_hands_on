/************************************************************************************************
* Workspaceとgitの連携のためのAPI Integration
************************************************************************************************/
...
SELECT
    AI_PARSE_DOCUMENT(
      TO_FILE('@citibike.public.terms_stage','citibike_terms_of_service.docx'),
      OBJECT_CONSTRUCT('mode','LAYOUT','page_split',TRUE)
    ) AS out_doc;


//AI_PARSE_DOCUMENTの結果を、ターゲットテーブルへINSERT
INSERT INTO citibike.public.terms_table (doc_id, file_name, page_index, page_md, page_count)
WITH raw AS (
  SELECT
    AI_PARSE_DOCUMENT(
      TO_FILE('@citibike.public.terms_stage','citibike_terms_of_service.docx'),
...
parsed AS (
  SELECT
    -- JSONとしてパースを試み、失敗したら元の VARIANT/OBJECT を採用
    COALESCE(TRY_PARSE_JSON(TO_VARCHAR(out_doc)), out_doc)::VARIANT AS j
  FROM raw
)
SELECT
  'citibike_terms_v1'              AS doc_id,
  'citibike_terms_of_service.docx' AS file_name,
  page.value:index::int            AS page_index,
  page.value:content::string       AS page_md,
  parsed.j:metadata:pageCount::int AS page_count
FROM parsed,
     LATERAL FLATTEN(input => parsed.j:pages) AS page; //ネストを含むJSON配列を行に展開するSnowflakeの標準構文

//ターゲットテーブルの確認
select * from citibike.public.terms_table;


//チャンク化のターゲットテーブル作成
CREATE OR REPLACE TABLE citibike.public.terms_chunks (
  doc_id         STRING,
  file_name      STRING,
  page_index     INT,
  chunk_index    INT,            
...
//SPLIT_TEXT_RECURSIVE_CHARACTERでチャンク化してインサート
INSERT INTO citibike.public.terms_chunks
  (doc_id, file_name, page_index, chunk_index, chunk_text, chunk_char_len)
SELECT
  t.doc_id,
  t.file_name,
  t.page_index,
  ch.index::int                      AS chunk_index,
  TRIM(ch.value::string)             AS chunk_text,
  LENGTH(ch.value::string)::int      AS chunk_char_len
FROM citibike.public.terms_table AS t,
LATERAL FLATTEN(
  input => SNOWFLAKE.CORTEX.SPLIT_TEXT_RECURSIVE_CHARACTER(
             t.page_md,                      -- テキスト
             'markdown',                     -- フォーマット（Markdown）
             300,               -- チャンク長（文字数）
             30             -- オーバーラップ（文字数）
             -- , ARRAY_CONSTRUCT('\n\n','\n',' ','')  -- 必要なら明示セパレータ
          )
) AS ch
WHERE t.page_md IS NOT NULL
  AND TRIM(t.page_md) <> '';

//チャンク化テーブルの確認
select * from citibike.public.terms_chunks
order by page_index, chunk_index;

// 変更追跡は自動有効化されますが、上流テーブルでの明示的ONを推奨
ALTER TABLE citibike.public.terms_chunks SET CHANGE_TRACKING = TRUE;


//Cortex Search用のWarehouse作成
CREATE OR REPLACE WAREHOUSE cortex_search_wh 
WITH WAREHOUSE_SIZE='SMALL' AUTO_RESUME=TRUE AUTO_SUSPEND=60;

//Cortex Searchの作成
CREATE OR REPLACE CORTEX SEARCH SERVICE citibike.public.citibike_terms_search
ON chunk_text
ATTRIBUTES page_index, chunk_index  -- 返却/フィルタに使う属性列
WAREHOUSE = cortex_search_wh
TARGET_LAG = '5 hour'                                    -- インデックスの鮮度目標（更新頻度に応じ調整）
EMBEDDING_MODEL = 'snowflake-arctic-embed-l-v2.0'    
AS (
  SELECT
    page_index, chunk_index, chunk_text
  FROM citibike.public.terms_chunks
);

SELECT PARSE_JSON(
  SNOWFLAKE.CORTEX.SEARCH_PREVIEW(
    'citibike.public.citibike_terms_search',
    '{
      "query": "走行中に事故を起こしてしまいました。どうすればよいですか。",         
      "columns": ["chunk_text","chunk_index","page_index"], 
      "limit": 5
    }'
  )
)['results'] AS results;

//snowsightで作成されたCortex Searchを確認しよう
